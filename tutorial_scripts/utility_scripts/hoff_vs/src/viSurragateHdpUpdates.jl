...
function SurragateLowerBound_unconstrained_util(vec_args,T,e_γ,Tαk)
    c = vec_args[1, :] # rho_hat
    d = vec_args[2, :] # omega_hat
    calc_SurragateLowerBound_unconstrained(c,d,T,e_γ,Tαk)
end
...
function g_unconstrained!(GD, x, T,e_γ,Tαk)
    rho_hat =  x[1,:] # c
    omega_hat = x[2,:] # d
    ...
    minusrho_hat = (1 .- rho_hat)
    Δ_mk = genterate_Delta_mk(rho_hat,omega_hat)
    Δ_prod_T =  Δ_mk[:,1:K] .* Tαk[1:K]
    Δ_prod_Tαk = permutedims(sum(Δ_prod_T,dims=1))
    #take Negative to find max  

    GD[1,:] = 1.0 .*(rho_hat .* minusrho_hat .* (omega_hat .* (T .+ 1 .-  rho_omega_hat) .* polygamma.(1,rho_omega_hat) .- omega_hat .*( T .*(K .+ 1.0 .- k_vec)  .+ e_γ .- minusrho_omega_hat) .* polygamma.(1,minusrho_omega_hat) .+ Δ_prod_Tαk)) # c and rho
    #-2.0 .* (1.0 .- x[1,:]) .- 400.0 .* (x[2,:] .- x[1,:].^2) .* x[1,:] 
    GD[2,:] = 1.0 .* ( omega_hat .* ((T .+ 1.0 .- rho_omega_hat) .* (rho_hat .* polygamma.(1,rho_omega_hat) .- polygamma.(1,omega_hat)) .+ ( T .* (K .+ 1.0 .- k_vec) .+ e_γ .-  minusrho_omega_hat) .* ( minusrho_hat .* polygamma.(1, minusrho_omega_hat)  .- polygamma.(1,omega_hat) ))) # d and omega
     #(x[2,:] .- x[1,:].^2)
end
...
g_unconstrained_closure!(T,e_γ,Tαk) = (GD, x)  ->  g_unconstrained!(GD, x, T,e_γ,Tαk)
...
SurragateLowerBound_unconstrained_closure(T,e_γ,Tαk) = vec_args ->  SurragateLowerBound_unconstrained_util(vec_args,T,e_γ,Tαk)
